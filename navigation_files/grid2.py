...
## If height of flying is betweeen 2 walls
...
def quadrant_to_grid(arg1):
    ...
    if ((0<=x_s<=10) and (0<=y_s<=10))  :
        grid_y = math.floor(40+(x_s/10)*40)
        grid_x = math.floor(0 +((10-y_s)/10)*40)

    elif((-10<=x_s<=0) and (0<=y_s<=10)) :
        grid_y = math.floor(0+((10+x_s)/10)*40)
        grid_x = math.floor(0 +((10-y_s)/10)*40)

    elif((-10<=x_s<=0) and (-10<=y_s<=0))  :
        grid_y = math.floor(0+((10+x_s)/10)*40)
        grid_x = math.floor(40 +((-y_s)/10)*40)   
...
#will take gate argument but only [x,y] assumed the gate is of lengh 6 and (width=1m) down
#def shading_gates()   
...
print(g) #array for gates in list   
...
## coding for the new gates as obstacles 
#prepare the start and end in quad_map by approximation of 0.2 and 0.2 on either sides with an assumption that the position given is at center 
## manual tweeking for now can be more robust based on the heading angles
#gate1
st = np.zeros((4,2),dtype = float)
ed = np.zeros((4,2),dtype = float)

st[0] = [g1[0][0],g1[0][1]+0.2]
ed[0] = [g1[0][0],g1[0][1]-0.2]

st[1] = [g1[1][0]-0.2,g1[1][1]]
ed[1] = [g1[1][0]+0.2,g1[1][1]]

st[2] = [g1[2][0],g1[2][1]-0.2]
ed[2] = [g1[2][0],g1[2][1]+0.2]

st[3] = [g1[3][0]-0.2,g1[3][1]]
ed[3] = [g1[3][0]+0.2,g1[3][1]]



for i in range(len(g1)):
    a1=quadrant_to_grid(st[i])
    a2=quadrant_to_grid(ed[i])
    new_map=shading_grids(a1,a2,new_map)
...
gate_occupied_map = new_map

plt.imshow(gate_occupied_map)
plt.show()
